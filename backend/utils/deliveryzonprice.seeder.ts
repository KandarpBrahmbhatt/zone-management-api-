...
const TOTAL_RECORDS = 1_000_000; // 10 lakh
...
const generateData = (startIndex: number, batchSize: number, zones: any[]) => {
    ...
    for (let i = startIndex; i < startIndex + batchSize; i++) {
        ...
        data.push({
            postcodePrefixId: new mongoose.Types.ObjectId(
                zones[i % zones.length]._id,
            ),
            minWeight,
            maxWeight,
...
const seed = async () => {
    try {
        ...
        // optional fast clear
        await DeliveryZonePrice.collection.drop().catch(() => { });
